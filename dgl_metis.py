...
import sys
import dgl
from dgl.data import MetisGraphDataset
...
def train(model, device, train_loader, optimizer, epoch, best_rmse, best_mae):
    model.train()
    running_loss = 0.0
    for batch, subgraph in enumerate(train_loader):
        batch_nodes_u = subgraph.ndata['user_id']
        batch_nodes_v = subgraph.ndata['item_id']
        labels_list = subgraph.ndata['ratings']
        optimizer.zero_grad()
        ...
        if batch % 100 == 0:
            print('[%d, %5d] loss: %.3f, The best rmse/mae: %.6f / %.6f' % (
                epoch, batch, running_loss / 100, best_rmse, best_mae))
            running_loss = 0.0
...
def test(model, device, test_loader):
    ...
    with torch.no_grad():
        for subgraph in test_loader:
            test_u = subgraph.ndata['user_id'].to(device)
            test_v = subgraph.ndata['item_id'].to(device)
            tmp_target = subgraph.ndata['ratings'].to(device)
...
def main():
    ...
    # Define the DGL graph
    num_users = history_u_lists.__len__()
    num_items = history_v_lists.__len__()
    num_ratings = ratings_list.__len__()

    g = dgl.heterograph({
        ('user', 'rated', 'item'): (train_u, train_v),
        ('item', 'rated-by', 'user'): (train_v, train_u)
    })

    g.nodes['user'].data['user_id'] = torch.LongTensor(train_u)
    g.nodes['item'].data['item_id'] = torch.LongTensor(train_v)
    g.nodes['user'].data['ratings'] = torch.FloatTensor(train_r)

    # Define data loaders
    train_loader = GraphDataLoader(g, torch.arange(len(train_u)), batch_size=args.batch_size, shuffle=True)

    # Create your user, item, and rating embeddings

    # You will need to replace this with your own code for defining the embeddings
    # as well as the Node_Aggregator, Node_Encoder, and GraphConsis classes.
